...
public class RATCDeck : MonoBehaviour
{
   ...
   // public Dictionary<string, Sprite> dictSuits;
    ...
    // InitDeck organizes xml cards into game data
    public void InitDeck(string RatatatCatDeckText)
    {
    ...
    public void ReadDeck(string RatatatCatDeckText)
    {
        xmlr = new PT_XMLReader(); // Create a new PT_XMLReader
        xmlr.Parse(RatatatCatDeckText); // Use that PT_XMLReader to parse DeckXML
    ...
    {
        //each card is unique and is given a name
        // cardNames will be the names of cards to build
        // Each card has 3 of except 9 which has 9
        // (e.g.:
        // 1a,1b,1c, for the ones ;
        // 9a,9b,9c,9d,...9i, for the nines ;
        // 11a,11b,11c, for the peek cards ; 
        cardNames = new List<string>();
        string[] letters = new string[] { "a", "b", "c", "d", "e", "f", "g", "h", "i" };
        //assign letters a b and c to card names from number cards 1-8
        for(int j =0; j<3;j++)
        {
            string s = letters[j];
            //go through first 8 cards
            for (int i = 0; i < 9; i++)
            {
                cardNames.Add((i)+s);
            }
        }
        //for number card 9 go through all letters to assign
        ...
        //assign letters for numbers 10 through 12 
        for (int j = 0; j < 3; j++)
        {
            string s = letters[j];
            //go through numbers 10 through 12
            for (int i = 10; i < 13; i++)
            {
                cardNames.Add((i) + s);
            }
   ...
   //             _tSR.sprite = dictSuits[card.suit];
    ...
    private void AddPips(Card card)
    {
        ...
        foreach (Decorator pip in card.def.pips)
        {
            ...
            _tSR = _tGO.GetComponent<SpriteRenderer>();
            // Set the Sprite to the proper suit
 // no suits           _tSR.sprite = dictSuits[card.suit];
