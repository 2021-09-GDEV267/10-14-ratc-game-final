...
[System.Serializable]
public class RATCPlayer
{
    public PlayerType type = PlayerType.ai;
    public int playerNum;
    public SlotDef handSlotDef;
    public List<CardRATC> hand; // The cards in this player's hand

    // Add a card to the hand
    public CardRATC AddCard(CardRATC eCB)
    {
        if (hand == null) hand = new List<CardRATC>();
        ...
        {
            CardRATC[] cards = hand.ToArray();

            // This is the LINQ call
            cards = cards.OrderBy(cd => cd.suit).ThenBy(cdr => cdr.rank).ToArray();

            hand = new List<CardRATC>(cards);
    ...
    public CardRATC RemoveCard(CardRATC cb)
    {
    ...
    public void FanHand()
    {
        ...
        {
            rot = startRot - RatatatCat.S.handFanDegrees * i;
...
*/
    ...
    public void TakeTurn()
    {
       ...
       //  RatatatCat.S.phase = CurrentTurnPhase.waiting;

        CardBartok cb;

        // If this is an AI player, need to make a choice about what to play
        // Find valid plays
        List<CardRATC> validCards = new List<CardRATC>();
        foreach (CardRATC tCB in hand)
        {
            if (RatatatCat.S.ValidPlay(tCB))
            {
                validCards.Add(tCB);
            }
        }
        ...
        RemoveCard(cb);
        RatatatCat.S.MoveToTarget(cb);
        cb.callbackPlayer = this;
    }
    */
    public void CBCallback(CardRATC tCB)
    {
        Utils.tr("Player.CBCallback()", tCB.name, "Player " + playerNum);
        // The card is done moving, so pass the turn
        
        Debug.Log("RatatatCat.S.PassTurn()");
